...
## Fix status — PR #28 (4 revalidation rounds; latest R4 `f2b062c8`)

The maintainer (Cassandra Heart) landed **4 fix commits** on PR #28 in response to this audit, each independently revalidated by the audit-suite pipeline (static call-site tracing, augmented with an executed F009 simulation in R3 and a full build in R4): [R1 (883c4a5b)](REVALIDATION-883c4a5b.md), [R2 (b14378a2)](REVALIDATION-b14378a2.md), [R3 (cf62383e)](REVALIDATION-cf62383e.md), [R4 (f2b062c8)](REVALIDATION-f2b062c8.md). The per-finding bodies below are unchanged and reflect the **original OPEN severity at the audited base** (`6cff47c…`); this section is the overlay describing current state on the `pow` branch.

**Build arc:** R1 was buildable. **R2 failed to compile** (`error[E0061]` — F135 DA-PoW producer arity mismatch at `main.rs:1404`). **R3 failed to compile** (`error[E0277]` ×3 — F031 gRPC `with_interceptor` wiring passing `&MyHubService` where `T: HubService` is required, `main.rs:294-300`). **R4 compiles clean (nightly)** — `cargo +nightly check --bin hypersnap` → exit 0, the first buildable fix commit since R1. (Stable rustc 1.95.0 ICEs on the unchanged `ed448-bulletproofs` dependency, so verification used nightly 1.98.0 — an environmental toolchain issue, not a property of the commit.)

Fix status ∈ {FIXED, PARTIAL, OPEN, UNTOUCHED, not revalidated}. **Round** = the round whose verdict the row reflects. Findings the four reports do not adjudicate are marked **not revalidated** (no status invented). The roll-up is built from the four `REVALIDATION-*.md` reports in this directory, which are the source of truth.
...
| F024 | High | PARTIAL | R2 | Scheduler split-read FIXED R1; supervisor anchor-jump catch-up loop FIXED R2 (`dkls_supervisor.rs:131-133`); two narrow residuals OPEN — current-epoch cold-start skip + single-`Option` watchdog tracking only the last epoch of a burst. |
| F026 | High | PARTIAL | R4 | Future-epoch leak FIXED R1; `DifferentEpochs` slashing bypass FIXED R2; read-path index/epoch resolution FIXED R4 (enforced set). BUT the `transport_pubkey_for_party`/`peer_id_for_party` twins still resolve over the RAW set — OPEN (R4 missed them inside the same fix). |
...
| F004 | High | PARTIAL | R2 | One race closed R2 (`refresh_proposer_context_loop` single-snapshot); deferred races (cutover offset, two desync anchor sources, `build_driver` re-read) UNTOUCHED R3/R4. |
