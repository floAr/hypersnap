...
### [F138 — Proposer broadcast strips locks/transfers + zeros signed anchor metadata; chain halts on any honest multi-node deployment](findings/drafts/F138-proposer-pipeline-strips-locks-transfers-and-signed-anchor-fields-from-wire-broadcast.md)
...
### [F133 — `FingerprintStore` writes bypass txn_batch on the gRPC simulate path; per-validator divergence → consensus fork via unauthenticated RPC](findings/drafts/F133-fingerprint-store-direct-db-writes-during-simulate-cause-fork-and-free-poisoning.md)
...
### [F091 — Cross-FID `lock_id` collision lets any attacker permanently strand a victim's bridge-locked balance](findings/drafts/F091-lock-id-collision-across-fids-permanently-strands-victim-balance.md)
...
### [F105 — App-PoW receipts have no epoch / timestamp binding; one captured receipt replays every future epoch indefinitely](findings/drafts/F105-app-usage-receipt-no-epoch-binding-cross-epoch-replay.md)
...
### [F108 — DKLS signing trusts inner `parties.sender` for routing + abort-blame; 1-packet panic-DoS + misattribution-blame against any reachable peer](findings/drafts/F108-dkls-signing-trusts-inner-sender-for-routing-and-blame.md)
...
### [F132 — `RewardStore::stage_charge_message_fee` reads accumulators from disk, not from the in-progress batch; same-FID fee-bearing messages silently free](findings/drafts/F132-stage-charge-message-fee-read-after-write-collapse.md)
...
### [F107 — DKLS `step5` skips DLog verification for any `ProofCommitment` whose inner `index` equals the verifier's own `party_index`](findings/drafts/F107-dkls-step5-skips-verification-for-self-claimed-proof-commitment-index.md)
...
### [F114 — DKG zero-share init trusts inner `parties.sender`/`receiver` bytes; one-packet DoS / misattribution-blame / silent ZeroShare-vec corruption](findings/drafts/F114-dkls-zero-share-init-trusts-inner-parties-sender-receiver.md)
**HC 0.85 · rust-threshold-signing**
...
### [F116 — KZG trusted-setup loader silently treats Lagrange-basis G1 points as monomial powers of τ](findings/drafts/F116-kzg-loader-assumes-monomial-basis-no-lagrange-detection-or-conversion.md)
...
### [F151 — `SnapchainCodec` decode panics on peer Vote/Proposal/Commits before signature verify](findings/drafts/F151-snapchain-codec-decode-panics-on-peer-vote-proposal-and-syncresponse.md)
...
### [F153 — Hyperblock threshold-ECDSA signing payload omits `signer_indices` → attacker-controlled slashing](findings/drafts/F153-hyperblock-threshold-sig-omits-signer-indices-extra-rules-payload-attacker-controlled-slashing.md)
**WP 0.92 · rust-threshold-signing**
...
### [F117 — Verkle insertions for locks omit the 1-byte domain discriminator that nullifier/note-commitment inserts use](findings/drafts/F117-verkle-lock-key-missing-domain-byte-path-prefix-panic-dos.md)
...
### [F119 — `DLogProof::verify` panics on malformed challenge length → 1-packet remote-thread panic-DoS](findings/drafts/F119-dlogproof-verify-panic-on-malformed-challenge-length.md)
**WP 0.95 · rust-crypto-primitives**
...
### [F135 — DA-PoW driver pads `served_key` to 32 bytes; apply-path exact-byte trie lookup never matches → §5 DA-PoW reward signal = 0 across the fleet](findings/drafts/F135-da-pow-driver-zero-pads-served-key-breaking-apply-time-trie-existence-gate.md)
...
### [F154 — Farcaster v2 batch endpoints accept unbounded `fids` array + uncapped per-FID pagination loop → multi-GB heap DoS via single unauth request](findings/drafts/F154-farcaster-batch-endpoints-unbounded-fid-list-and-uncapped-pagination-loop.md)
**WP 0.90 · p2p-gossip**
...
### [F158 — JFS-signed notification webhook events have no app_id/nonce/timestamp/destination binding → cross-app replay (notification phishing under trusted-app identity)](findings/drafts/F158-jfs-webhook-no-app-id-or-nonce-binding-enables-cross-app-replay.md)
**WP 0.93 · p2p-gossip**
...
### [F101 — Custody-key JFS account-association proof is a publicly-served, replayable bearer token (no chain-id, no nonce, no consumption)](findings/drafts/F101-account-association-jfs-proof-replayable-no-chain-or-nonce-binding.md)
...
### [F121 — IPA `from_bytes` uses `Scalar::from_bytes_mod_order` instead of `from_canonical_bytes` → confidential-transfer wire-byte malleability](findings/drafts/F121-ipa-from-bytes-uses-mod-order-not-canonical-prover-wire-malleability.md)
...
### [F149 — Transfer codec `one_time_pubkey` + `blinding_diff_scalar` not signed → relay-attacker output-burn griefing](findings/drafts/F149-transfer-codec-one-time-pubkey-and-blinding-diff-not-signed-malleability-burns-recipient-output.md)
**HC 0.85 · rust-crypto-primitives**
...
### [F157 — `following_fid` filter on mini-app send endpoint enumerates an attacker-chosen FID's entire follower set per request → shared-RocksDB DoS](findings/drafts/F157-following-fid-filter-unbounded-follower-enumeration.md)
**WP 0.88 · p2p-gossip**
...
### [F094 — Bridge-burn watcher resume cursor is derived from the drainable `BridgeBurnStore` queue, not from a persisted high-watermark](findings/drafts/F094-bridge-burn-watcher-cursor-derived-from-drainable-queue.md)
**WP 0.92 · solidity-bridge**
...
### [F096 — `apply_inbound_burn` short-circuits on `(source_chain_id, burn_id)` nullifier BEFORE signature verification — accepts & one-hop-broadcasts forged unsigned messages](findings/drafts/F096-inbound-burn-nullifier-short-circuit-bypasses-signature-verification.md)
...
### [F097 — `recovery_watcher` omits finality wait, has poisonable cursor, and uses panicking U256-to-u64 narrowing](findings/drafts/F097-recovery-watcher-missing-finality-wait-and-poisonable-cursor.md)
**WP 0.93 · solidity-bridge**
...
### [F104 — `FeeDepositBody` Ed25519 signing payload omits `chain_id` → replayable across hypersnap deployments / shards (F101-class variant)](findings/drafts/F104-fee-deposit-no-chain-id-binding-replayable-across-hypersnap-shards.md)
...
### [F095 — `BridgeBurnStore` watermark is poisonable and queue is never pruned, degrading the inbound-bridge over time](findings/drafts/F095-watermark-poisoning-and-unbounded-queue-in-bridge-burn-store.md)
...
### [F137 — Importer's per-transfer re-validation in `runtime::import_block` omits the `extract_output_pubkeys` gate that proposer-side admission enforces](findings/drafts/F137-importer-re-validation-omits-extract_output_pubkeys-gate-and-silently-strands-outputs.md)
...
### [F110 — DKLS `refresh_phase4` and `refresh_complete_phase4` invoke the F107-defective `step5`; latent because `refresh.rs` is unreachable from production](findings/drafts/F110-dkls-refresh-step5-verification-skip-variant-of-F107.md)
