...
**Build arc:** R1 was buildable. **R2 failed to compile** (`error[E0061]` — F135 DA-PoW producer arity mismatch at `main.rs:1404`). **R3 failed to compile** (`error[E0277]` ×3 — F031 gRPC `with_interceptor` wiring passing `&MyHubService` where `T: HubService` is required, `main.rs:294-300`). **R4 compiles clean (nightly)** — the first buildable fix commit since R1. **R5 compiles clean (nightly).** **R6 compiles clean (nightly)** — `cargo +nightly check --bin hypersnap` → exit 0, 13m19s. (Stable rustc 1.95.0 ICEs on the unchanged `ed448-bulletproofs` dependency, so verification used nightly 1.98.0 — an environmental toolchain issue, not a property of the commit.)

**✅ R6 closes the R5 broken-fix — runtime-verified, no blocker remains.** R5's F004 cutover-offset fix left the runtime's authoritative `epoch_resolver` cutover-unaware, a PoC-confirmed launch-day chain halt (`produce_signed_block_dkls_local → Err(NoDklsShare)` at any mainnet cutover ≥ `EPOCH_LENGTH`). R6 applies the one-line fix at `runtime.rs:339` (`EpochManager::with_cutover(config.cutover_snapchain_block)`) and ships a regression test — `cutover_aware_resolver_reports_epoch_zero_post_cutover` — that exercises real `apply_cutover` + `produce_signed_block_dkls_local` at a mainnet-shaped cutover and **passes** (the inverse of our R5 PoC; it would have gone red on R5). R6 also collapses the F004 dual-anchor desync into one shared `LatestAnchor`, makes the F009 L2 distribution-aware, digest-binds the F023 phase-3 broadcast arm, and changes the default RPC bind to loopback. See [R6 report](REVALIDATION-a1e866ab.md) and [REMAINING-AFTER-R6](REMAINING-AFTER-R6.md).

Fix status ∈ {FIXED, PARTIAL, OPEN, UNTOUCHED, not revalidated}. **Round** = the round whose verdict the row reflects. Findings the reports do not adjudicate are marked **not revalidated** (no status invented). The roll-up is built from the six `REVALIDATION-*.md` reports in this directory, which are the source of truth.
...
| F004 | High | FIXED | R6 | `refresh_proposer_context_loop` race closed R2; `build_driver` double-read closed R5. R5's cutover-offset attempt was a PoC-confirmed broken-fix (resolver left cutover-unaware → launch-day `NoDklsShare` halt). **R6 fixes it**: `runtime.rs:339` now `EpochManager::with_cutover(config.cutover_snapchain_block)`, with a regression test (`cutover_aware_resolver_reports_epoch_zero_post_cutover`) that runs real `apply_cutover` + `produce_signed_block_dkls_local` at cutover=5,000,000 and asserts `current_epoch()==0` + production succeeds — **passes** (would have gone red on R5). R6 also collapses the dual-anchor desync into a single shared `LatestAnchor`. See [REMAINING-AFTER-R6](REMAINING-AFTER-R6.md). |
| F009 | High | FIXED | R6 | Three-layer defense; executed sim (R3) measured ring trust 0.0057 (≈9× below the 0.05 L0 floor) → ring growth 0.0. R3 caveat: L2 was distribution-blind. **R6 makes L2 distribution-aware** — entropy damping `(1−H_norm)^2` over per-crediter contributions; an executed probe confirms a uniform sybil ring is damped to 0 while a skewed real user survives. New low-severity caveat: the axis is contribution *uniformity*, not sybil-ness, so a legitimate creator with equal-weight engagement is damped identically to a ring (fairness, not security — L0 remains the real sybil gate; `skew_exponent=0` reverts). |
| F023 | High | FIXED | R6 | Cross-digest sign routing closed via `build_sign_aad(...)` on encrypted rounds + clobber guard (R2/R3). R3 caveat: phase-3 plaintext-broadcast arm (`receiver=None`) was digest-unbound. **R6 closes it** — new `DISCRIMINATOR_SIGN_BROADCAST` frame `[disc][digest_32B][raw]`; receiver rejects cross-digest/truncated frames before deserialization (2 codec tests pass). Residual (acknowledged in-code): structural binding, not cryptographic (digest is public) → bounded liveness-grief only; forgery blocked by `mul_sid` abort + F018. |
| F031 | High | FIXED | R4 | HTTP per-request limiter R2; gRPC limiter design correct R3 but did not compile; R4 wiring (`InterceptedService::new(HubServiceServer::from_arc(...), interceptor)`) compiles and gates all 4 methods. Residual: gRPC auth off-by-default — **R6 mitigates exposure** by changing the default RPC/HTTP bind from `0.0.0.0` to `127.0.0.1` (loopback-only out-of-the-box; auth remains opt-in). |
