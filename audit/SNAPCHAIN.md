...
#### [F002 — `add_proposed_value` unwraps peer-controlled `FullProposal` Option fields before validation](findings/F002-nil-block-proposal.md)
...
#### [F005 — `dispatch_decided_value` unwraps `DecidedValue` value chain on peer-controlled gossip input](findings/F005-read-validator-protocol-version.md)
...
#### [F151 — `SnapchainCodec` decode panics on peer Vote / Proposal / SyncResponse fields before signature verify](findings/F151-snapchain-codec-decode-panics-on-peer-vote-proposal-and-syncresponse.md)
...
#### [F017 — Gossipsub mesh has no peer scoring, no `validate_messages()`, default mesh_n_low / outbound_min, 100/100 connection limits](findings/F017-gossipsub-mesh-no-peer-scoring.md)
...
#### [F019 — No per-topic gossip message size cap; only the 10 MB libp2p transport cap is enforced](findings/F019-no-per-topic-gossip-size-cap.md)
...
#### [F021 — Autodiscovery `handle_contact_info` dials body-supplied address without binding to libp2p sender peer-id; `PeerId::from_bytes().unwrap()` panics on malformed bytes](findings/F021-autodiscovery-contact-info-not-bound-to-libp2p-peer-id.md)
...
#### [F030 — Unbounded HTTP body buffered via `Incoming::collect().await` with no transport-level cap](findings/F030-unbounded-body-buffer-pre-cap.md)
...
#### [F031 — No rate limit on any HTTP / gRPC ingress endpoint](findings/F031-no-rate-limit-on-http-grpc-ingress.md)
...
#### [F033 — Block / shard-chunk header committed in a SEPARATE RocksDB commit from state-mutation batch (cross-CF atomicity break)](findings/F033-shard-chunk-header-split-from-state-commit.md)
